...
/// Input sizes aligned with experiments/hashbench/configs/default.toml.
const INPUT_SIZES: &[usize] = &[
    8, 16, 32, 64, 256, 512, 1024, 4096, 16384, 65536, 131072, 1048576,
];
...
fn throughput_benches(c: &mut Criterion) {
    ...
    for &pattern in PATTERNS {
        for &size in INPUT_SIZES {
            let input = generate_input(size, pattern, SEED);
            let mut group = c.benchmark_group(format!("throughput/{pattern}/{}B", size));
...
fn latency_benches(c: &mut Criterion) {
    ...
    for &pattern in PATTERNS {
        for &size in INPUT_SIZES {
            let input = generate_input(size, pattern, SEED);
            let mut group = c.benchmark_group(format!("latency/{pattern}/{}B", size));
